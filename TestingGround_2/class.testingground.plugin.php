...
class TestingGround extends Gdn_Plugin {
	...
	public function PostController_Render_Before($Sender) {
		// Only insert on new discussions
		if($Sender->RequestMethod == 'discussion') {
			$this->_AddResources($Sender);
			// Get the variables from the URL through the Request object provided by Garden
			$Request = $Sender->Request->Get();
			// Sanitize the input to prevent some SQL injection attacks
			$Title = filter_var($Request['title'], FILTER_SANITIZE_ENCODED);
			$Tag = filter_var($Request['tag'], FILTER_SANITIZE_ENCODED);
			// Adding the variables as definitions will give our JS access to the filtered data
			$Sender->AddDefinition('TG_DiscussionTitle', $Title);
			$Sender->AddDefinition('TG_DiscussionTag', $Tag);
		}
	}
