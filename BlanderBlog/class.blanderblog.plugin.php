...
class BlanderBlog extends Gdn_Plugin {
	...
	// Adds the class 'Blog' to every discussion in the blog category list. Allows for themes to style the blog independently of the plugin.
	public function CategoriesController_BeforeDiscussionName_Handler($Sender) {
		if (in_array($Sender->CategoryID, C('Plugins.BlanderBlog.CategoryIDs'))) {
			echo 'CSS Added on categories controller.';
			$Sender->EventArguments['CssClass'] .= ' Blog Blog'.$Sender->CategoryID.' ';
		}
	}

	// Adds the class 'Blog' to every comment (including the first post) in the blog category list. Allows for themes to style the blog independently of the plugin.
	public function DiscussionController_BeforeCommentDisplay_Handler($Sender) {
		if (in_array($Sender->CategoryID, C('Plugins.BlanderBlog.CategoryIDs'))) {
			echo 'CSS Added on discussion controller.';
			$Sender->EventArguments['CssClass'] .= ' Blog Blog'.$Sender->CategoryID.' ';
		}
	}
	...
	public function DiscussionModel_BeforeGet_Handler($Sender) {
		...
		if (array_key_exists('d.CategoryID', $Wheres) && in_array($Wheres['d.CategoryID'][0], C('Plugins.BlanderBlog.CategoryIDs'))) {
			$Sender->EventArguments['SortField'] = 'd.DateInserted';
			$Sender->EventArguments['SortDirection'] = 'desc';
		}
	}
	
	// Override the default discussion view with a customizable single post blog view. Allows for themes to override the blog's html output.
	...
	// Override the default category view with a customizable blog view. Allows for themes to override the blog's html output.
